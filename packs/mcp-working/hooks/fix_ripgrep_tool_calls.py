...
2. Hallucinated tool name variations (exec → bash)
...
async def fix_ripgrep_tool_calls(ctx: HookContext) -> None:
    ...
    2. Corrects hallucinated tool name variations (exec* → bash)
    """
    ...
    for tool_id, tool_call in message.tool_calls.items():
        ...
        if original_name in TOOL_NAME_CORRECTIONS:
            ...
            tool_call.params.name = "bash"
            logger.warning(
                "Corrected unknown exec* variant to bash",
                data={"tool_id": tool_id, "original": original_name, "corrected": "bash"},
            )
